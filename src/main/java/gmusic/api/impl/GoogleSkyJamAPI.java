...
public class GoogleSkyJamAPI extends GoogleMusicAPI implements IGoogleSkyJam {
    ...
    private final Collection<Track> getTracks(String continuationToken)
                                       throws ClientProtocolException, IOException, URISyntaxException {
        Collection<Track> chunkedCollection = new ArrayList<Track>();
        TrackFeed chunk =
            JSON.Deserialize(client.dispatchPost(new URI("https://www.googleapis.com/sj/v1beta1/trackfeed"), "{\"start-token\":\"" + continuationToken + "\"}"),
                             TrackFeed.class);
        chunkedCollection.addAll(chunk.getData().getItems());
    ...
    @Override
    public Collection<File> downloadTracks(Collection<Track> tracks)
