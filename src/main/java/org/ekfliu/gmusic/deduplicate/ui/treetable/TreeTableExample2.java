...
public class TreeTableExample2 {
    ...
    protected void updateStatusLabel() {
        if (reloadPath != null) {
            ...
            statusLabel.setText("Total Size: " + NumberFormat.getInstance().format(model.getTotalSize(model.getRoot())));
    ...
    class IndicatorRenderer extends DefaultTableCellRenderer {
        ...
        public void paint(Graphics g) {
            if (lastRow == reloadRow) {
                ...
                if (reloadCounter < 5) {
                    g.fillArc((width - diameter) / 2, (height - diameter) / 2, diameter, diameter, 90, -(reloadCounter * 90));
                } else {
                    g.fillArc((width - diameter) / 2, (height - diameter) / 2, diameter, diameter, 90, (4 - (reloadCounter % 4)) * 90);
                }
            } else {
                super.paint(g);
            }
